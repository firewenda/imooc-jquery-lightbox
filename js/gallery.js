...
function addPhotos() {
	var _wrap = '';
	var _nav = '';
	for (var i = 0; i < dataList.length; i++) { //for in 循环中的循环计数器是字符串，而不是数字它包含当前属性的名称或当前数组元素的索引
		_wrap += '<div class="photo  photo_front" onclick="turn(this)" id="photo_' + (i + 1) + '"><div class="photo-wrap"><div class="side side-front"><p class="image"><img src="' + dataList[i].img + '"></p><p class="caption">' + dataList[i].caption + '</p></div><div class="side side-back"><p class="desc">' + dataList[i].desc + '</p></div></div></div></div>';
	...
	rsort(random([1, dataList.length]));
}
